...
const MedicalReportGenerator = () => {
  ...
  const [testResults, setTestResults] = useState([
    { testName: "Hemoglobin (Hb)", result: "14.2", unit: "g/dL", refRange: "13.5–17.5", status: "Normal" },
    { testName: "WBC Count", result: "8,500", unit: "cells/µL", refRange: "4,000–11,000", status: "Normal" },
    { testName: "Platelets", result: "210,000", unit: "/µL", refRange: "150,000–450,000", status: "Normal" },
    { testName: "Fasting Glucose", result: "110", unit: "mg/dL", refRange: "70–99", status: "High" },
    { testName: "Serum Creatinine", result: "1.4", unit: "mg/dL", refRange: "0.7–1.3", status: "High" },
    { testName: "ALT (Liver Enzyme)", result: "50", unit: "IU/L", refRange: "8–37", status: "High" },
    { testName: "TSH", result: "2.1", unit: "mIU/L", refRange: "0.4–4.0", status: "Normal" },
  ])
...
• Kidney and liver function monitoring`)
  ...
  const generateGeminiInterpretation = async () => {
    ...
    try {
      ...
      const abnormalTests = testResults.filter((test) => test.status === "High" || test.status === "Low")
      const testSummary = testResults
        .map((test) => `${test.testName}: ${test.result} ${test.unit} (Ref: ${test.refRange}) - ${test.status}`)
        .join("\n")
...
${abnormalTests.map((test) => `${test.testName}: ${test.result} ${test.unit} - ${test.status}`).join("\n")}`
      ...
      setInterpretation("Unable to generate AI interpretation at this time. Please review results manually.")
      setComments("Please consult with your healthcare provider for proper interpretation of these results.")
    }
  ...
  const updateTestResult = (index: number, field: TestResultField, value: string) => {
    ...
    <div className="min-h-screen bg-gray-100 p-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPatientData({ ...patientData, sampleId: e.target.value })}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPatientData({ ...patientData, collectionDate: e.target.value })}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setPatientData({ ...patientData, reportDate: e.target.value })}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>
          ...
          <div className="mb-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-3">Lab Information</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setLabData({ ...labData, addressLine: e.target.value })}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">City, State - Pincode</label>
                <input
                  type="text"
                  value={labData.cityPincode}
                  onChange={(e) => setLabData({ ...labData, cityPincode: e.target.value })}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
            </div>
              ...
              <table className="w-full border-collapse border border-gray-300">
                <thead>
                  <tr className="bg-gray-100">
                    <th className="border border-gray-300 px-3 py-2 text-left text-sm font-semibold">Test Name</th>
                    <th className="border border-gray-300 px-3 py-2 text-left text-sm font-semibold">Result</th>
                    <th className="border border-gray-300 px-3 py-2 text-left text-sm font-semibold">Unit</th>
                    ...
                    <th className="border border-gray-300 px-3 py-2 text-left text-sm font-semibold">Action</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={index}>
                      ...
                      <td className="border border-gray-300 px-3 py-2">
                        <input
                          type="text"
                          value={test.refRange}
                          onChange={(e) => updateTestResult(index, "refRange", e.target.value)}
                          className="w-full border-none focus:outline-none text-sm"
                        />
                      </td>
                      <td className="border border-gray-300 px-3 py-2">
                      ...
                      <td className="border border-gray-300 px-3 py-2">
                        <button onClick={() => deleteTestResult(index)} className="text-red-600 hover:text-red-800">
        ...
        <div className="bg-white shadow-lg" id="report-content">
          ...
          <div className="border-b-2 border-black p-6">
            <div className="flex justify-between items-start">
              ...
              <div className="text-right">
                <p className="text-xs text-gray-600">Report ID: {patientData.sampleId}</p>
                <p className="text-xs text-gray-600">Date: {patientData.reportDate}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <span className="ml-2 font-semibold">{patientData.age} years</span>
                </div>
                <div>
                  <span className="text-gray-600">Gender:</span>
                  <span className="ml-2 font-semibold">{patientData.gender}</span>
                </div>
                <div>
                  <span className="text-gray-600">Sample ID:</span>
                  <span className="ml-2 font-semibold">{patientData.sampleId}</span>
                </div>
                <div>
                  <span className="text-gray-600">Collection Date:</span>
                  <span className="ml-2 font-semibold">{patientData.collectionDate}</span>
                </div>
                <div>
                  <span className="text-gray-600">Report Date:</span>
                  <span className="ml-2 font-semibold">{patientData.reportDate}</span>
                </div>
              </div>
            </div>
            ...
            {/* Test Results */}
            <div className="mb-6">
              <h2 className="text-sm font-bold text-gray-800 mb-3 border-b border-gray-200 pb-1">LABORATORY RESULTS</h2>
                ...
                <tbody>
                  {testResults.map((test, index) => (
                    <tr key={index} className={index % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                      <td className="border border-gray-300 px-3 py-2 text-xs">{test.testName}</td>
                      <td className="border border-gray-300 px-3 py-2 text-xs font-semibold">{test.result}</td>
                      <td className="border border-gray-300 px-3 py-2 text-xs">{test.unit}</td>
                      <td className="border border-gray-300 px-3 py-2 text-xs">{test.refRange}</td>
                      <td
                        className={`border border-gray-300 px-3 py-2 text-xs font-semibold ${
                          test.status === "High" || test.status === "Low" ? "text-red-600" : "text-green-600"
                        }`}
                      >
                        {test.status}
                      </td>
            ...
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-2">
                <h2 className="text-sm font-bold text-gray-800 border-b border-gray-200 pb-1">INTERPRETATION</h2>
                <button
                  onClick={() => setIsEditingInterpretation(!isEditingInterpretation)}
                  className="text-blue-600 hover:text-blue-800 no-print"
                >
                  {isEditingInterpretation ? <Save className="w-3 h-3" /> : <Edit3 className="w-3 h-3" />}
                </button>
              </div>
              {isEditingInterpretation ? (
                ...
                <div className="bg-gray-50 border border-gray-200 p-3">
                  <pre className="whitespace-pre-wrap text-xs text-gray-800 leading-relaxed">{interpretation}</pre>
                </div>
              )}
            </div>
            ...
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-2">
                <h2 className="text-sm font-bold text-gray-800 border-b border-gray-200 pb-1">COMMENTS</h2>
                <button
                  onClick={() => setIsEditingComments(!isEditingComments)}
                  className="text-blue-600 hover:text-blue-800 no-print"
                >
                  {isEditingComments ? <Save className="w-3 h-3" /> : <Edit3 className="w-3 h-3" />}
                </button>
              </div>
              {isEditingComments ? (
                <textarea
                ...
                <div className="bg-gray-50 border border-gray-200 p-3">
                  <pre className="whitespace-pre-wrap text-xs text-gray-800 leading-relaxed">{comments}</pre>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="border-t border-gray-200 p-6">
            <div className="flex justify-between items-end">
              <div>
                <p className="text-sm font-semibold text-gray-800 mb-1">{labData.name}</p>
                <p className="text-sm font-medium text-gray-700">{labData.addressLine}</p>
            ...
            <div className="border-t border-gray-300 mt-4 pt-3 text-center">
              <p className="text-xs text-gray-600">
                This report is computer generated and does not require signature. For queries, visit redtestlabs.com
              </p>
            </div>
          </div>
        </div>
