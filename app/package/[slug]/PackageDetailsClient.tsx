...
export default function PackageDetailsClient({ slug }: { slug?: string }) {
  ...
  const handleShareClick = async () => {
    const title = packageData?.name || "Redtestlab Package"
    const text = "Check out this health package on Redtestlab"
    const url = currentUrl
    if (navigator.share) {
      try {
        await navigator.share({ title, text, url })
        toast({ title: "Shared", description: "Thanks for sharing this package." })
        return
      } catch (err: any) {
  ...
  const handleCopyLink = async () => {
    try {
      ...
      toast({ title: "Link copied", description: "Package URL copied to clipboard." })
      setShareOpen(false)
    } catch (err) {
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-3 gap-3 mb-3">
                          <Button asChild variant="ghost" className="h-auto py-3 flex flex-col items-center gap-2 border rounded-lg hover:bg-gray-50">
                            <a
                              href={`https://wa.me/?text=${encodeURIComponent((packageData?.name || "") + " " + currentUrl)}`}
                              target="_blank"
